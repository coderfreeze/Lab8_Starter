...
self.addEventListener('fetch', function (event) {
  ...
  // First check is this a precached asset?
  const isPrecachedRequest = RECIPE_URLS.includes(event.request.url); 
  ...
  if (isPrecachedRequest) {
    ...
    // go to network -> add to cache -> return network response
    event.respondWith(caches.open(CACHE_NAME).then((cache) => {
